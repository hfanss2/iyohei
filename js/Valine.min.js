...
{return function(e)
	{function n(r)
		...
		<a rid='"+i.id+"' at='@"+i.get("nick")+'\' class="vat">回复</a>\n                                        </div>';var l=n.el.querySelector(".vlist"),s=l.querySelectorAll("li"),c=a.querySelector(".vat"),u=a.querySelectorAll("a");for(var v in u)if(u.hasOwnProperty(v)){var d=u[v];"at"!=d.getAttribute("class")&&(d.setAttribute("target","_blank"),d.setAttribute("rel","nofollow"))}n.bindAt(c),l.insertBefore(a,s[1])}}n.loading.hide()}).catch(function(e){n.loading.hide()}),n.bind()}},
		{key:"bind",value:function(){var e=this,n={veditor:"comment",vnick:"nick",vlink:"link",vmail:"mail"},t={};for(var r in n)n.hasOwnProperty(r)&&function(){var i=n[r],a=e.el.querySelector("."+r);t[i]=a,p.on("input",a,function(e){c[i]=m.encode(a.value.replace(/(^\s*)|(\s*$)/g,""))})}();var i=function(){var n=v&&v.getItem("ValineCache");if(n){n=JSON.parse(n);
		var t=["nick","link","mail"];for(var r in t){var i=t[r];e.el.querySelector(".v"+i).value=n[i],c[i]=n[i]}}};i(),e.reset=function(){for(var t in n)if(n.hasOwnProperty(t)){var r=n[t],a=e.el.querySelector("."+t);a.value="",c[r]=""}c.rid="",
		c.nick="小可爱",i()};var a=e.el.querySelector(".vsubmit"),s=function(n){if(a.getAttribute("disabled"))return void e.alert.show({type:0,text:'再等等，评论正在提交中ヾ(๑╹◡╹)ﾉ"',ctxt:"好的"});if(""==c.comment)return void t.comment.focus();
		if(""==c.nick&&(c.nick="小调皮"),c.comment=(0,o.default)(c.comment),c.comment.indexOf(c.at)>-1&&""!=c.at){var r='<a class="at" href=\'#'+c.rid+"'>"+c.at+"</a>";c.comment=c.comment.replace(c.at,r)}var i=f.mail(c.mail),l=f.link(c.link);i.k||0===c.link.length||l.k?i.k?0===c.link.length||l.k?(c.mail=i.v,c.link=l.v,d()):(c.link="",c.mail=i.v,e.alert.show({type:1,text:"您的网址格式不正确, 是否继续提交?",cb:function(){d()}})):(c.mail="",c.link=l.v,e.alert.show({type:1,text:"您的邮箱格式不正确, 是否继续提交?",cb:function(){d()}})):(c.mail="",c.link="",e.alert.show({type:1,text:"您的网址和邮箱格式不正确, 是否继续提交?",cb:function(){d()}}))},u=function(){var n=new e.v.ACL;return n.setPublicReadAccess(!0),n.setPublicWriteAccess(!1),n},d=function(){a.setAttribute("disabled",!0),e.loading.show();var n=e.v.Object.extend("Comment"),t=new n;for(var r in c)if(c.hasOwnProperty(r)){if("at"===r)continue;var i=c[r];t.set(r,i)}t.set("emailHash",l(c.mail.toLowerCase().trim())),t.setACL(u()),t.save().then(function(n){v&&v.setItem("ValineCache",
		JSON.stringify({nick:c.nick,link:c.link,mail:c.mail}));var t=e.el.querySelector(".count");t.innerText=Number(t.innerText)+1;var r=document.createElement("li");r.setAttribute("class","vcard"),r.setAttribute("id",n.id);var i="https://gravatar.cat.net/avatar/"+n.get("emailHash")+"?size=96";r.innerHTML='<img class="vavatar" src="'+i+'"/>\n                                    <div class="text-wrapper">\n                                    <div class="vhead" >\n                                    <a href="'+(n.get("link")||"javascript:void(0);")+'" target="_blank" rel="nofollow" >'+
